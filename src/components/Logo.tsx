  ...
  if (logoType === 'text-only') {
    return (
      <div className={`flex flex-col ${className}`}>
        <span className={`font-bold tracking-wide header-text ${size === 'sm' ? 'text-base' : size === 'md' ? 'text-xl' : size === 'lg' ? 'text-2xl' : 'text-4xl'} ${colorClasses[color]}`}>
          Mukamba
        </span>
        <span className={`font-semibold tracking-wider header-text ${size === 'sm' ? 'text-sm' : size === 'md' ? 'text-lg' : size === 'lg' ? 'text-xl' : 'text-2xl'} ${color === 'white' ? 'text-gray-100' : color === 'dark' ? 'text-gray-700' : 'text-red-600'}`}>
          GATEWAY
        </span>
      </div>
    );
